...
const Landing = props => {
  ...
  const [code, setCode] = useState(props.code || "//Write your code here")
  const [customInput, setCustomInput] = useState("")
  const [outputDetails, setOutputDetails] = useState(null)
  const [processing, setProcessing] = useState(null)
  const [showSaveCode, setShowSaveCode] = useState(false)
  const [codeTitle, setCodeTitle] = useState(props.title || "")
        ...
        <div className="w-full md:w-4/12 p-4 ml-auto">
          ...
          <div className="flex flex-col items-end">
            ...
            <div className="flex w-full justify-between">
              {status === "authenticated" && !props.codeId && (
                <button
                  className={`${!code &&
                    "opacity-50"} mt-4 font-black border-2  z-10 rounded-md  px-4 py-2 hover:bg-gray-400 transition duration-200 bg-white flex-shrink-0`}
              ...
              {status === "authenticated" && props.codeId && (
                <button
                  className={`${!code &&
                    "opacity-50"} mt-4 font-black border-2 z-10 rounded-md border-white  px-4 py-2 hover:bg-gray-100 transition duration-200 bg-gray-300 flex-shrink-0`}
                  onClick={() => setShowSaveCode(true)}
                  disabled={!code}
                >
                  Update code
              ...
              <button
                ...
                disabled={!code}
                className={`mt-4 bg-gradient-to-r from-nav via-violet-600 to-violet-500 text-white font-black border-2  z-10 rounded-md ] px-4 py-2 hover:shadow transition duration-200 bg-white flex-shrink-0 
            ${!code && "opacity-50"}`}
              >
          ...
          <Modal isOpen={showSaveCode} onClose={() => setShowSaveCode(false)}>
            ...
            <ModalContent>
              ...
              <ModalBody>
                <form >
                  ...
                  <input
                    ...
                    className="w-full border-2 border-black border-solid rounded-md  focus:outline-none p-3"
                    required
                  />
                  ...
                  <textarea
                    name="code"
                    value={code}
                    className="w-full border-2 border-black border-solid rounded-md  focus:outline-none p-3 resize-none"
                    disabled
                  />
                </form>
              </ModalBody>
              <ModalFooter>
                <button
                  className="mr-4 font-black border-2 border-gray-500 z-10 rounded-md  px-4 py-2 hover:shadow transition duration-200 bg-white flex-shrink-0"
                ...
                {props.update ? (
                  <button
                    ...
                    className="font-black border-2 border-gray-500 z-10 rounded-md  px-4 py-2 hover:shadow transition duration-200 bg-white flex-shrink-0"
                  >
                    Update
                  </button>
                ) : (
